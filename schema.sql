...
CREATE TABLE quick_replies(_id INTEGER PRIMARY KEY AUTOINCREMENT,title TEXT UNIQUE NOT NULL,content TEXT NOT NULL, associated_label_ids TEXT);
...
CREATE TABLE group_notification_version(group_jid_row_id INTEGER PRIMARY KEY,subject_timestamp INTEGER NOT NULL,announcement_version INTEGER NOT NULL,participant_version INTEGER NOT NULL, group_join_request_timestamp INTEGER, group_photo_timestamp INTEGER);
...
CREATE TRIGGER message_bd_for_interactive_message_header_content_trigger BEFORE DELETE ON message BEGIN DELETE FROM interactive_message_header_content WHERE message_row_id=old._id; END;
CREATE TRIGGER message_bd_for_interactive_message_sections_trigger BEFORE DELETE ON message BEGIN DELETE FROM interactive_message_sections WHERE message_row_id=old._id; END;
CREATE INDEX interactive_message_header_content_message_row_id_index
...
CREATE TRIGGER chat_bd_for_aea_chat_state_trigger
            ...
            BEGIN
              ...
              WHERE
                chat_row_id=old._id;
            END;
CREATE TABLE newsletter_media_clear(_id INTEGER PRIMARY KEY AUTOINCREMENT,chat_row_id INTEGER NOT NULL,media_type INTEGER NOT NULL,cutoff_sort_id INTEGER NOT NULL);
CREATE TRIGGER chat_bd_for_newsletter_media_clear_trigger BEFORE DELETE ON chat BEGIN DELETE FROM newsletter_media_clear WHERE chat_row_id=old._id; END;
